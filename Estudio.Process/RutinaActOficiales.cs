...
namespace Estudio.Process
{
    class RutinaActOficiales
    {
        ...
        public beResultados RutinaPension(
            List<beDatosModalidad> ModelCot, List<beDatosBen> ListaBen,
            ...
            List<beTasasPromedio> ModelTasPro, List<beCurvaTasas> ModelTasCurva, string banderaPanta, int asesor, string banderaWS, string casoEx)
        {
            ...
            double prccomS, prcfaclab;

            List<beDatosBen> ModelBen = ListaBen;
            int hijo, madre;

            hijo = (from b in ListaBen where b.CodPar == "30" select b).Count();

            madre = (from b in ListaBen where b.CodPar != "99" select b).Count();

            if (madre > 2)
            {
                if (hijo != 0 && ListaBen[2].CodPar == "10")
                {
                    ListaBen[2].CodPar = "11";
                }
            }
            ...
            foreach (var item in ModelCot)
            {
                ...
                if (vlVecesCot == 1)
                {
                    ...
                    foreach (var itemben2 in ModelBen)
                    {
                        ...
                        Ncorbe[l] = int.Parse(CopiaBen[l].CodPar);
                        if (TipPen == "S")
                        {
                            if (Ncorbe[l] == 99)
                            {
                ...
                foreach (var itemGas in TGfil)
                {
                    ...
                    if (TipRen == "D")
                    {
                        //tasac_t = Convert.ToDouble(item.Prc_Minimo_1);             //'Tasa de Costo Capital
                        tasac_t = Convert.ToDouble(item.Prc_Minimo);
                    }
                    else
                    {
                        //tasac_t = Convert.ToDouble(item.Prc_Minimo);
                        tasac_t = Convert.ToDouble(item.Prc_Minimo_1);
                    }
                    //tasac_t = itemGas.PrcTir;             //'Tasa de Costo Capital
                ...
                //                for (ax = 0; ax <= mescon - 1; ax++)
                //                {
                //                    FecCot2 = new DateTime(Nap, Nmp, 1);
                //                    FxVac = ModelFacVac.Where(x => x.FEC_IPC == FecCot2).Select(x => x.MTO_IPC).SingleOrDefault();
                //                    sumCostosN = 0;
                //                    sumaCostTotalMes = 0;
                //                    for (j = 0; j <= Nben; j++)
                //                    {
            ...
            CalTva:
                ...
                dUtilImp = salcta_eva - (dComis + gasemi + dGasMan + 0 + dVarRes); //PR
                ...
                for (i = 2; i <= nmax; i++)
                {
                    ...
                    {
                        flupag = dflupag;
                        flujo2 = penanu * Flupen[iRes - 1] + Flucm[iRes - 1];
                    }
                    else
                    {
                        flupag = penanu * Flupen[iRes - 1] + Flucm[iRes - 1];
                        flujo2 = penanu * Flupen[iRes] + Flucm[iRes];
                    }
                    /*//PR FIN*/
                    //flupag = penanu * Flupen[iRes] + Flucm[iRes]; //PR

                    for (long a = iRes; a <= nmax; a++) //PR
                    {
                        resfin = resfin + ((penanu * Flupen[a + 1] + Flucm[a + 1]) / (Math.Pow((1 + tce), at)));
                        at = at + 1;
                    }
                    //resfin = flupag + resfin; //PR
                    resfin = flujo2 + resfin; //PR
            ...
            CalTce2:
                ...
                dGasMan = (gastos / 12);
                dVarRes = reserva;
                //dUtilImp = salcta_eva - (dComis + gasemi + dGasMan + dflupag + dVarRes); //PR
                dUtilImp = salcta_eva - (dComis + gasemi + dGasMan + 0 + dVarRes);  //PR
                dvarCap = dVarRes * dMarSol;
            ...
            CalExd2:
                ...
                if (TipPen == "C" || TipPen == "M")
                {
                    ...
                    else
                    {
                        ...
                        Resultados.MTO_VALPREPENTMP = Math.Round((vld_saldo * Valmon) / (sumapenben * Valmon), 2);
            ...
            _log.Info("Datos Resultado: PENSION: " + ListaResultador.MTO_PENSION + " COTEST " + ListaResultador.NUM_COTESTUDIO + " CORR " + ListaResultador.NUM_CORRELATIVO);
            _log.Info("Datos Resultado: TIR: " + ListaResultador.PRC_TASATIR + " COTEST " + ListaResultador.NUM_COTESTUDIO + " CORR " + ListaResultador.NUM_CORRELATIVO);
            _log.Info("Datos Resultado: TASAVTA: " + ListaResultador.PRC_TASAVTA + " COTEST " + ListaResultador.NUM_COTESTUDIO + " CORR " + ListaResultador.NUM_CORRELATIVO);
            _log.Info("Datos Resultado: PERDIDA: " + ListaResultador.PRC_PERCON + " COTEST " + ListaResultador.NUM_COTESTUDIO + " CORR " + ListaResultador.NUM_CORRELATIVO);
            _log.Info("Datos Resultado: PRIMAUNICA: " + ListaResultador.PRIMA_UNICA + " COTEST " + ListaResultador.NUM_COTESTUDIO + " CORR " + ListaResultador.NUM_CORRELATIVO);
            _log.Info("Datos Resultado: MTO_PRIMA: " + ListaResultador.MTO_PRIMA + " COTEST " + ListaResultador.NUM_COTESTUDIO + " CORR " + ListaResultador.NUM_CORRELATIVO);
