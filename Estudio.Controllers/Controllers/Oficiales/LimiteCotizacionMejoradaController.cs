...
{
    public class LimiteCotizacionMejoradaController : Controller
    {
        LimiteCotizacionMejoradaLogic _LimiteCotizacionMejoradaLogic = new LimiteCotizacionMejoradaLogic();
        ...
        public ActionResult Index()
        {
            ...
            mensaje = "";
            ViewBag.Departamentos = new SelectList(_LimiteCotizacionMejoradaLogic.TipoDepartamento(), "ClaveDepartamento", "Elemento");
            ViewBag.TipoMoneda = new SelectList(_LimiteCotizacionMejoradaLogic.TiposMoneda(), "ClaveMoneda", "Elemento");
            ViewBag.Periodos = new SelectList("");
            ViewBag.RangoTasa = new SelectList(_LimiteCotizacionMejoradaLogic.RangosTasa(), "IdRangoTasa", "Elemento");
        ...
        public ActionResult ListaPeriodos(string vlMoneda, int vlReajuste, int departamento)
        {
            try
            {
                var resultado = _LimiteCotizacionMejoradaLogic.ListaPeriodos(vlMoneda, vlReajuste, departamento);
        ...
        public ActionResult Consulta(DateTime fechaIni, string codMoneda, int reajuste, int departamento)
        {
            try
            {
                var resultado = _LimiteCotizacionMejoradaLogic.Consulta(fechaIni, codMoneda, reajuste, departamento);
        ...
        public ActionResult GrabarCorizacionMejorada(LimiteCotizacionMejorada informacion, DateTime fechaIni,Boolean bandera)
        {
            try
            {
                string usuario = Convert.ToString(this.Session["Account"]);
                var resultado = _LimiteCotizacionMejoradaLogic.Grabar(informacion,fechaIni, bandera, usuario);
        ...
        /// <param name="departamento">valor del departamento</param>
        /// <returns>retorna un mensaje en pantalla despues del proceso</returns>
        public ActionResult EliminarCorizacionMejorada(DateTime fechaIni, string codMoneda, int reajuste, string departamento)
        {
            try
            {
                string usuario = Convert.ToString(this.Session["Account"]);
                var resultado = _LimiteCotizacionMejoradaLogic.EliminarCorizacionMejorada(fechaIni, codMoneda, reajuste, departamento, usuario);
        ...
        {//Informe de Parámetros de Tasa de Anclaje
            var resultExportacion = _LimiteCotizacionMejoradaLogic.ConsultaRpt(codMoneda, reajuste, moneda, departamento, nomDepartamento, fechaInicial);

            ReportDocument rpt = new ReportDocument();
            rpt.Load(Server.MapPath("~/Resources/reports/Pt_Rpt_ParCotizacionMej.rpt"));
        ...
        private string UploadRecordsToDataBase(string fileName, string tipoRenta)
        {
            ...
            using (var stream = System.IO.File.Open(Path.Combine(Server.MapPath("~/Files/"), fileName), FileMode.Open, FileAccess.Read))
            {
                using (var reader = ExcelReaderFactory.CreateReader(stream))
                {
                    ...
                    List<LimiteCotizacionMejorada> listDatos = new List<LimiteCotizacionMejorada>();

                    for (int i = 0; i < regiones.Length; i++)
                    {
                        List<LimiteCotizacionMejorada> codReg = _LimiteCotizacionMejoradaLogic.getCodRegion(regiones[i]);
                        if (codReg.Count != 0)
                        {
                            obtenerInfoRegion(listDatos, reader, rowCount, codReg, colIni[i], regiones[i]);
                        }
                    }
                    return _LimiteCotizacionMejoradaLogic.gurdarDatosMasivos(listDatos, usuario, tipoRenta).Message;
                }
            }
        }

        public void obtenerInfoRegion(List<LimiteCotizacionMejorada> listDatos, IExcelDataReader xlRange, int rowCount, List<LimiteCotizacionMejorada> codReg, int colIni, string lugar)
        {
            try
            {
                ...
                for (int i = 4; i <= rowCount; i++) //renglon
                {
                    ...
                    LimiteCotizacionMejorada dato = new LimiteCotizacionMejorada();
                    dato.lugar = lugar;
        ...
        public void definirReg(LimiteCotizacionMejorada dato, List<LimiteCotizacionMejorada> codReg, string cic, int indice)
        {
            dato.cic = cic;
