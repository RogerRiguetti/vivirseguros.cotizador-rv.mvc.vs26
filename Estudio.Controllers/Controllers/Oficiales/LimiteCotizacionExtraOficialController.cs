...
{
    public class LimiteCotizacionExtraOficialController : Controller
    {
        LimiteCotizacionExtraOficialLogic _LimiteCotizacionExtraOficiallLogic = new LimiteCotizacionExtraOficialLogic();
        ...
        public ActionResult Index()
        {
            ...
            mensaje = "";
            ViewBag.Departamentos = new SelectList(_LimiteCotizacionExtraOficiallLogic.TipoDepartamento(), "ClaveDepartamento", "Elemento");
            ViewBag.TipoMoneda = new SelectList(_LimiteCotizacionExtraOficiallLogic.TiposMoneda(), "ClaveMoneda", "Elemento");
            ViewBag.Periodos = new SelectList("");
            ViewBag.RangoTasa = new SelectList(_LimiteCotizacionExtraOficiallLogic.RangosTasa(), "IdRangoTasa", "Elemento");
        ...
        public ActionResult ListaPeriodos(string vlMoneda, int vlReajuste, int departamento)
        {
            try
            {
                var resultado = _LimiteCotizacionExtraOficiallLogic.ListaPeriodos(vlMoneda, vlReajuste, departamento);
        ...
        public ActionResult Consulta(DateTime fechaIni, string codMoneda, int reajuste, int departamento)
        {
            try
            {
                var resultado = _LimiteCotizacionExtraOficiallLogic.Consulta(fechaIni, codMoneda, reajuste, departamento);
        ...
        public ActionResult GrabarCorizacionInicial(LimiteCotizacionExtraOficial informacion, DateTime fechaIni, Boolean bandera)
        {
            try
            {
                string usuario = Convert.ToString(this.Session["Account"]);
                var resultado = _LimiteCotizacionExtraOficiallLogic.Grabar(informacion, fechaIni, bandera, usuario);
        ...
        public ActionResult EliminarCorizacionInicial(DateTime fechaIni, string codMoneda, int reajuste, string departamento)
        {
            ...
            {
                string usuario = Convert.ToString(this.Session["Account"]);
                var resultado = _LimiteCotizacionExtraOficiallLogic.EliminarCorizacionInicial(fechaIni, codMoneda, reajuste, departamento, usuario);
        ...
        {//Informe de Parámetros de Tasa de Anclaje
            var resultExportacion = _LimiteCotizacionExtraOficiallLogic.ConsultaRpt(codMoneda, reajuste, moneda, departamento, nomDepartamento, fechaInicial);
        ...
        private string UploadRecordsToDataBase(string fileName)
        {
            ...
            using (var stream = System.IO.File.Open(Path.Combine(Server.MapPath("~/Files/"), fileName), FileMode.Open, FileAccess.Read))
            {
                using (var reader = ExcelReaderFactory.CreateReader(stream))
                {
                    ...
                    List<LimiteCotizacionExtraOficial> listDatos = new List<LimiteCotizacionExtraOficial>();

                    for (int i = 0; i < regiones.Length; i++)
                    {
                        List<LimiteCotizacionExtraOficial> codReg = _LimiteCotizacionExtraOficiallLogic.getCodRegion(regiones[i]);
                        if (codReg.Count != 0)
                        {
                            obtenerInfoRegion(listDatos, reader, rowCount, codReg, colIni[i], regiones[i]);
                        }
                    }
                    return _LimiteCotizacionExtraOficiallLogic.gurdarDatosMasivos(listDatos, usuario).Message;
                }
            }
        }

        public void obtenerInfoRegion(List<LimiteCotizacionExtraOficial> listDatos, IExcelDataReader xlRange, int rowCount, List<LimiteCotizacionExtraOficial> codReg, int colIni, string lugar)
        {
            try
            {
                ...
                for (int i = 4; i <= rowCount; i++) //renglon
                {
                    ...
                    LimiteCotizacionExtraOficial dato = new LimiteCotizacionExtraOficial();
                    dato.lugar = lugar;
        ...
        public void definirReg(LimiteCotizacionExtraOficial dato, List<LimiteCotizacionExtraOficial> codReg, string cic, int indice)
        {
            dato.cic = cic;
